...
async function processObject(object, archaeodoxUrl, accessToken, tagPoolMapping) {
    ...
    const ensembleName = object.ensemble.bezeichnung;

    const existingEnsembles = await findExistingEnsembles(archaeodoxUrl, districtName, ensembleName, accessToken);
    if (existingEnsembles.length) return;

    const poolId = getPoolId(object, tagPoolMapping);
    if (poolId) return await createEnsembleInArchaeodox(districtName, ensembleName, poolId, archaeodoxUrl, accessToken);
}

async function findExistingEnsembles(archaeoDoxUrl, districtName, ensembleName, accessToken) {
        ...
        search: [
            {
                type: 'match',
                bool: 'must',
                fields: ['ensemble.dgis_gemarkung'],
                string: districtName
            },
            {
                type: 'match',
                bool: 'must',
                fields: ['ensemble.dgis_ensemble_name'],
                string: ensembleName
            },
        ]
    };
...
async function createEnsembleInArchaeodox(districtName, ensembleName, poolId, archaeodoxUrl, accessToken) {
    const object = buildEnsembleObject(districtName, ensembleName, poolId);
    return await saveObject(object, archaeodoxUrl, accessToken);
}

function buildEnsembleObject(districtName, ensembleName, poolId) {
    return {
        _objecttype: 'ensemble',
        _mask: 'ensemble__all_fields',
        ensemble: {
            dgis_gemarkung: districtName,
            dgis_ensemble_name: ensembleName,
            _pool: {
                pool: {
                    _id: poolId
                }
